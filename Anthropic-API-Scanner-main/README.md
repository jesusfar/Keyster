...
2. Install required pypi packages
    ...
    ```bash
    pip install selenium tqdm anthropic rich fastapi uvicorn python-dotenv pytest pytest-cov
    ```
...
You can also query the scanned keys using the built-in REST API:
...
|----------|-------------|
| `GET /keys` | Get all keys (optional `?status_filter=yes`) |
| `GET /keys/available` | Get all working keys |
| `GET /keys/{api_key}` | Get details for a specific key |
| `POST /keys/{api_key}/check` | Re-check a key's status |
| `GET /stats` | Get statistics about scanned keys |
