...
def check_key(key, model="claude-3-5-haiku-20241022") -> str | None:
    """
    ...
    try:
        ...
        # Check specifically for insufficient quota (usually 400 or 429 depending on API state, but error code gives it away)
        if "credit balance is too low" in str(e).lower() or "insufficient_quota" in str(e).lower():
            rich.print(f"[dark_orange]insufficient_quota ({e.status_code})[/dark_orange]: '{key[:10]}...{key[-10:]}'")
