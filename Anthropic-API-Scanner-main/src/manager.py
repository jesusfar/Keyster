...
class DatabaseManager:
    ...
    def __enter__(self):
        ...
        self.cur.execute("CREATE INDEX IF NOT EXISTS idx_apikey ON APIKeys(apiKey)")
        self.cur.execute("CREATE INDEX IF NOT EXISTS idx_apikey_status ON APIKeys(status)")
        self.cur.execute("CREATE INDEX IF NOT EXISTS idx_url ON URLs(url)")
    ...
    def export_all_keys(self) -> list:
        """
        Get all keys with their statuses and last checked date for exporting.
        
        Returns:
            list: A list of tuples containing (apiKey, status, lastChecked).
        """
        if self.cur is None:
            raise ValueError("Cursor is not initialized")
        self.cur.execute("SELECT apiKey, status, lastChecked FROM APIKeys WHERE status='yes'")
        return self.cur.fetchall()
